...
package object p/*<=flags.p.package.*/ {
  ...
  ???/*=>scala.Predef.`???`().*/ match { case List/*=>scala.collection.immutable.List.*/(xs2/*<=local11*/) => ???/*=>scala.Predef.`???`().*/ }
  ???/*=>scala.Predef.`???`().*/ match { case _: List/*=>scala.package.List#*/[t/*<=local12*/] => ???/*=>scala.Predef.`???`().*/ }
}

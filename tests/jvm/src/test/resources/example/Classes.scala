...
object N/*<=classes.N.*/ {
  val anonClass/*<=classes.N.anonClass.*/ = new C7/*=>classes.C7#*/(42) {
    val local/*<=local26*/ = ???/*=>scala.Predef.`???`().*/
  }
  val anonFun/*<=classes.N.anonFun.*/ = List/*=>scala.collection.immutable.List.*/(1).map/*=>scala.collection.immutable.List#map().*/ { i/*<=local32*/ =>
    val local/*<=local33*/ = 2
    local/*=>local33*/ +/*=>scala.Int#`+`(+4).*/ 2
  }
}
